    ...
    overall.tankStats.map((row) => {
        if (snapshotTanks.length < index + 1 || row[0] !== snapshotTanks[index][0]) {
            ...
            let vehicleRecentStats = {
                id: row[0],
                name: tankNames[row[0]]['short_name'] + (tankNames[row[0]]['is_premium'] ? " 🟊" : ""),
                nation: nationConversion[tankNames[row[0]]['nation']],
                tier: tankNames[row[0]]['tier'],
                class: Conversion[tankNames[row[0]]['type']],
                battles: row[1],
                winrate: (winrate).toFixed(2),
                wn8: parseInt(WN8),
                dpg: parseInt(avgDamage), 
                kpg: (avgFrag).toFixed(2),
                dmgRatio: (row[2]/row[8]).toFixed(2),
                kd: (row[4]/destroyed).toFixed(2),
                spots: avgSpots.toFixed(2),
                survived: (row[7]*100/row[1]).toFixed(2) + '%', 
                isPrem: tankNames[row[0]]['is_premium']              
            };
            let raw = {
                id: row[0],
                battles: row[1],
