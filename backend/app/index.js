...
cron.schedule("30 2 * * *", function() {
    if (PRODUCTION === "TRUE") {
        ...
        fetch(`https://tomatoupdater-etiag.ondigitalocean.app/update/eu`);
    }
});
...
cron.schedule("30 12 * * *", function() {
    if (PRODUCTION === "TRUE") {
        ...
        fetch(`https://tomatoupdater-etiag.ondigitalocean.app/update/com`);
    }
});
...
cron.schedule("30 22 * * *", function() {
    if (PRODUCTION === "TRUE") {
        ...
        fetch(`https://tomatoupdater-etiag.ondigitalocean.app/update/asia`);
    }
});

cron.schedule("0 20 * * *", function() {
    console.log("Running Daily MoE Values 3PM EST");
    const servers = ['com', 'eu', 'ru', 'asia'];
    servers.forEach((server) => MoETracker(server));
});
